    ...
    def test_set_node(self):
        with config_mock():
            with unittest.mock.patch('src.views.node_selection.RPCServer.check_readiness', return_value=True):
                ...
                node_selection.node.set('http://localhost:38081')
                self.app.update()
                self.assertEqual(node_selection.node.get(), "http://localhost:38081")
                ...
                self.assertEqual(self.context.rpc_server._daemon_address(), 'http://localhost:38081')

    def test_set_random_node(self):
        with config_mock():
            with unittest.mock.patch('src.views.node_selection.RPCServer.check_readiness', return_value=True):
                with vcr.use_cassette('test/fixtures/cassettes/test_set_random_node.yaml'):
                    with unittest.mock.patch('src.views.node_selection.get_random_node', return_value='http://ldn1.derp.anstee.dev:18081'):
                        with unittest.mock.patch('src.views.history.TransactionsScrollableFrame.update_txs', return_value=True):
                            ...
                            self.app.update()
                            self.assertEqual(node_selection.node_selection.get(), 'http://ldn1.derp.anstee.dev:18081')
                            node_selection.submit_button._canvas.event_generate("<Button-1>")
                            self.context.server_ready()
                            self.app.update()
                            self.assertEqual(self.app.current_view, self.app.views['main'])
                            self.assertEqual(self.context.rpc_server._daemon_address(), 'http://ldn1.derp.anstee.dev:18081')
